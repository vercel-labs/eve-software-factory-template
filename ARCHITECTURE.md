...
- **Label intake can't be forged by reporters.** Only the `labeled` webhook action dispatches; a label pre-set at issue creation (which issue templates allow unauthenticated users to do) does not. Applying a label requires triage permission on the repo.
...
- **Approval prompts on GitHub:** the channel ships no built-in renderer for `input.requested`, so the github channel declares its own handler that posts a comment when a session stops for input: the pending tool and input, and the reply incantation. Answers route back through the same `onComment` mention gate, which is what makes a comment reply an authorization signal rather than a race anyone on a public repo can win: an untrusted account's "approve" never reaches the waiting session.
